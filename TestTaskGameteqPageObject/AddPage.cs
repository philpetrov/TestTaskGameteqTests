...
namespace PageObject
{
    public class AddPage : BasePage //страница создания приложения (ввода параметров)
    {
        ...
        private IWebElement FieldName => driver.FindElement(By.CssSelector("input[placeholder='name']")); // Поле "name" - уникальный селектор
        ...
        private IWebElement BtnSave => driver.FindElement(By.XPath("//span[text()=' Save ']")); // Кнопка "Savet" - уникальный путь




        //МЕТОДЫ
        public void SendTextToName(string text) => FieldName.SendKeys(text); //метод ввода текста в поле "name"
        public void SendTextToKey(string text) => FieldKey.SendKeys(text); //метод ввода текста в поле "key"
